...
url = URI("https://football-player1.p.rapidapi.com/calcio")
...
request["X-RapidAPI-Key"] = api_key
request["X-RapidAPI-Host"] = 'football-player1.p.rapidapi.com'

response = http.request(request)
responsed = JSON.parse(response.read_body)
# puts responsed[0]["nome"]
puts "Creating players"

100.times do |i|
  puts "..."
  file = URI.open("#{responsed[i]["immagine"]}")
  player = Player.create!(name: "#{responsed[i]["nome"]}")
